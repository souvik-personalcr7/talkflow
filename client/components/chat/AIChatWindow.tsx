...
export default function AIChatWindow({ onBack }: AIChatWindowProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-gray-50">
      {/* Header */}
      <div className="h-16 border-b border-gray-200 bg-white flex items-center px-4 justify-between sticky top-0 z-10 shadow-sm">
      ...
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-lg">
              {["What is React?", "Explain JWT", "Help me understand MongoDB", "Explain Socket.IO"].map((suggestion, i) => (
                <button
                  ...
                  className="px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 hover:border-purple-300 hover:bg-purple-50 transition-all text-left shadow-sm font-medium"
                >
                  {suggestion}
                </button>
              ))}
            ...
            if (isUser) {
              return (
                <div key={msg.id} className="flex w-full justify-end mb-4">
                  <div className="max-w-[75%] px-4 py-2.5 rounded-2xl bg-black text-white rounded-br-sm shadow-sm">
                    <p className="text-base leading-relaxed break-words whitespace-pre-wrap">{msg.content}</p>
                    <div className="text-xs mt-1 text-right flex justify-end items-center space-x-1 text-gray-400">
              ...
              return (
                <div key={msg.id} className="flex w-full justify-start mb-4">
                  <div className="max-w-[85%]">
                    <div className={`px-4 py-3 rounded-2xl border text-gray-900 shadow-sm ${
                      msg.status === 'error' ? 'bg-red-50 border-red-200 rounded-bl-sm' : 'bg-white border-gray-200 rounded-bl-sm'
                    }`}>
                      {msg.status === 'error' ? (
                      ...
                      <div className="text-xs mt-2 flex justify-between items-center text-gray-500 border-t border-gray-100 pt-2">
        ...
        {requestState === 'sending' && (
          <div className="flex w-full justify-start mb-4">
            <div className="px-4 py-3 rounded-2xl bg-white border border-gray-200 text-gray-900 rounded-bl-sm shadow-sm flex items-center space-x-2">
      ...
      {isGenerating && (
        <div className="absolute bottom-24 left-0 right-0 flex justify-center pointer-events-none z-20">
          <button
            ...
            className="pointer-events-auto flex items-center space-x-2 bg-white border border-gray-200 shadow-md text-gray-700 text-sm font-medium px-4 py-2 rounded-full hover:bg-gray-50 transition-colors"
          >
            <Square size={14} className="fill-gray-700" />
      ...
      {/* Input Area */}
      <div className="p-4 bg-white border-t border-gray-200 shadow-sm z-10 relative">
        <div className="flex items-end space-x-2 max-w-4xl mx-auto">
          <div className="flex-1 bg-gray-50 border border-gray-200 rounded-2xl flex items-center overflow-hidden focus-within:ring-2 focus-within:ring-black focus-within:border-transparent transition-all">
            <textarea
              value={inputText}
              onChange={(e) => setInputText(e.target.value)}
              onKeyDown={handleKeyDown}
              placeholder="Ask TalkFlow AI..."
              className="w-full max-h-32 min-h-[44px] py-3 px-4 bg-transparent resize-none focus:outline-none text-base text-gray-900"
              rows={1}
